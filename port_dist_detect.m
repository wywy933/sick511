...
while 1
    ...
    maxdis = 0;
    for i = 2:1:length(dis)-1
        if dis(i) ~= 0 && dis(i-1) ~= 0 && dis(i+1) ~= 0
            if abs(dis(i) - dis(i - 1)) > maxdis
                maxdis = abs(dis(i) - dis(i - 1));
                i
                [dis(i) dis(i - 1)]
                maxnum = (i-1);
                maxatr = (i-1) * angle_step;
                maxatd = (i-1) * angle_step * 180 / pi;
            end
        end
    end
    ...
    polar([0 maxatr],[0 max(dis)],'r')
%     polar([0 maxatr + angle_step],[0 max(dis)])
    title(['max distance at',num2str(maxatd),'with value of',num2str(maxdis)]);
    pause(0.001);
end
